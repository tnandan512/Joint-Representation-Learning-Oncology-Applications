...
# Load Methylation data for GBM and LGG samples
methyl = pd.read_csv(
    "<path_to_methyl_data>"  # User should replace with the path to BraTS T2-FLAIR feature data
)
...
D1 = geodesic_dist(t2f_data, k=30, metric="minkowski", mode="distance")
# Compute geodesic distances for Methylatiom data
D2 = geodesic_dist(methyl_data, k=50, metric="minkowski", mode="distance")
D1 = torch.from_numpy(
    D1
).float()  # Convert the T2-FLAIR distance matrix to a PyTorch tensor
D2 = torch.from_numpy(
    D2
).float()  # Convert the Methyl distance matrix to a PyTorch tensor
...
JMDS = JointMDS(
    ...
    alpha=0.1,  # Regularization parameter for joint optimization
...
Z2 = pd.DataFrame(
    Z2, index=methyl.index, columns=["Dimension_1", "Dimension_2"]
...
# Create labels for the embeddings: 0 for T2-FLAIR, 1 for Methyl
labels_X1 = np.zeros(Z1.shape[0])  # Label for T2-FLAIR features
labels_X2 = np.ones(Z2.shape[0])  # Label for Methyl features
