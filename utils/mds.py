...
def _smacof_single(
    dissimilarities,
    n_components=2,
    init=None,  # initial config of low-dim points (optional)- if provided, algorithm starts from here
    weights=None,
    Vplus=None,
    max_iter=300,
    eps=1e-3,
...
def smacof(
    ...
    init=None,
    weights=None,
    n_init=8,  # no. of times algorithm is run w different initial config.
    n_jobs=None,
    max_iter=300,
    eps=1e-3,
):
    ...
    for it in range(
        n_init
    ):  # multiple-run strategy increases chance of finding better solution - not get stuck in local minima
