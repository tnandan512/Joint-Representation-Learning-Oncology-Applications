...
def sinkhorn(  # solve OT problem i.e. update P
    C,
    a=None,
    b=None,
    eps=1.0,
...
def inv_ot(
    X1, X2, a=None, b=None, eps=0.0, max_iter=10, tol=1e-05
):  # update P then O iteratively
    m, d1 = X1.shape
    n, d2 = X2.shape

    O = torch.eye(d1).to(
        X1
    )  # creates an identity matrix w dim d1 x d1 and same dtype as X1
    ...
    if (
        a is None
    ):  # if a and b not provided, they are initialised to uniform distribution
        a = X1.new_ones((m,)) / m
    if b is None:
        b = X2.new_ones((n,)) / n
    v_ot = None
    ...
    for i in range(max_iter):
        # fixed O, optimise P using sinkhorn
        dot = torch.mm(X1.mm(O), X2.T)  # m x n, dot product between ZO and Z'
        dist = norms2 - 2 * dot
        P, v_ot = sinkhorn(dist, a=a, b=b, v=v_ot, return_v=True, eps=eps, max_iter=5)

        # update O - fix P and find O using SVD
        O = X1.T.mm(P.mm(X2))
        u, s, v = torch.linalg.svd(O)
        O = u.mm(v)

        if torch.norm(P_old - P) < tol:  # checking for convergence here
            break
        P_old = P
...
### Gromov-Wasserstein - used to initialise first P matrix
...
# used to initialise P matrix, here referred to as T
